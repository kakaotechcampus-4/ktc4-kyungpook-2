...
역할은 진입 경로로 정해집니다. 이미 가입한 회원의 역할은 다시 로그인해도 바뀌지 않습니다.
...
로그인 응답에는 바디가 없습니다. 로그인 직후 상태(`userId` · `role` · `name` 등)는
[§2.2 `GET /api/v1/auth/me`](#22-세션-조회--라우트-가드가-매-진입마다-호출)로 확인합니다.
