...
def route_after_shortlist(state: MatchingState) -> Literal["llm_judge", "decide"]:
    ...
    # 후보가 아예 없으면 모델이 할 수 있는 일이 없다
    if not candidates:
        return "decide"

    # 여러 아이가 등장했다면 대등 언급인지 확정해야 한다
    if state.get("co_mention"):
        return "llm_judge"

    # 이름이 그대로 적힌 아이 한 명뿐이면 코드로 충분하다
    if len(candidates) == 1 and candidates[0]["confidence"] >= EXACT_SCORE:
        return "decide"

    # 오타 의심 · 유사 이름 · 표지 힌트만 있는 경우 → 모델에 넘긴다
    return "llm_judge"
