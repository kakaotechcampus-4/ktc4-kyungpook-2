...
export default function OrgSettingsPage() {
  ...
  const [doc, setDoc] = useState({ name: "인가증_2026.pdf", status: "verified" as "verified" | "reviewing" });

  return (
    <>
      <PageHeader title="기관 설정" description="기관 정보와 증빙서류 검증 상태입니다" />
        ...
        <form className="flex flex-col gap-5">
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3 rounded border border-dashed border-line2 bg-paper px-4 py-4">
              <span className="text-[15px] text-ink2">{doc.name}</span>
              {doc.status === "verified" ? (
                <span className="rounded bg-passsoft px-2 py-0.5 text-[13px] font-semibold text-pass">
                  검증 완료
                </span>
              ) : (
                <span className="rounded bg-reviewsoft px-2 py-0.5 text-[13px] font-semibold text-review">
                  재검증 중
                </span>
              )}
              <input
                ...
                onChange={(e) => {
                  ...
                  if (file) setDoc({ name: file.name, status: "reviewing" });
                }}
              />
              <button
                type="button"
                onClick={() => fileInputRef.current?.click()}
                className="ml-auto tap rounded border border-line2 bg-surface px-3 text-[15px] font-semibold text-ink2"
              >
                다시 올리기
              </button>
            </div>
