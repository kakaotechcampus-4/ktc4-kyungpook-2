...
import { Card, EvidenceChip, Note, PageHeader } from "@/components/ui";
...
export async function clientLoader() {
  ...
  return { childList: children.filter((c) => c.status === "active") };
}

export default function ChatPage() {
  const { childList } = useLoaderData<typeof clientLoader>();
  ...
  return (
    <>
      <PageHeader title="상담 지원" description="승인된 기록에 근거해서만 답합니다" />
          ...
          <div key={i} className="flex flex-col gap-2">
            ...
            {t.answer ? (
                ...
                <ul className="flex flex-wrap gap-2">
                  {t.sources.map((s) => (
                    <li key={s.date + s.label}>
                      <EvidenceChip date={s.date} label={s.label} href={`/children/${childId}`} />
                    </li>
                  ))}
                </ul>
              </Card>
              ...
              <div className="self-start rounded border border-line2 bg-surface2 px-4 py-3">
                ...
                <p className="mt-1 text-[14px] text-muted">관련된 승인 기록이 아직 없습니다.</p>
              </div>
            )}
          </div>
        ))}
