...
import { generateInviteCode, registerChild } from "@/lib/api";
...
export default function ChildRegisterPage() {
  ...
  async function handleRegister(e: React.FormEvent<HTMLFormElement>) {
    ...
    setPending(true);
    try {
      const { inviteCode } = await registerChild({ name, birthDate });
      setRegistered({ code: inviteCode });
    } finally {
      setPending(false);
    }
  }

  const inviteLink = registered ? `https://itda.app/invite/${registered.code}` : "";
      ...
      <Card className="mb-5">
        <form onSubmit={handleRegister} className="flex flex-col gap-5">
          ...
          <button
            type="submit"
            disabled={pending}
            className="tap self-start rounded bg-accent px-5 text-[16px] font-semibold text-white hover:bg-accentink disabled:opacity-50"
          >
            {pending ? "등록 중…" : "등록하고 초대코드 발급"}
          </button>
        </form>
      </Card>
      ...
      {registered ? (
        <Card className="border-accent/40 bg-accentsoft">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              onClick={() => navigator.clipboard.writeText(registered.code)}
              className="tap rounded bg-accent px-4 text-[15px] font-semibold text-white"
            >
              코드 복사
            </button>
            <button
              onClick={() => navigator.clipboard.writeText(inviteLink)}
              className="tap rounded border border-accent/50 bg-surface px-4 text-[15px] font-semibold text-accentink"
            >
              초대 링크 복사
            </button>
            <button
              onClick={() => setRegistered({ code: generateInviteCode() })}
              className="tap rounded border border-line2 bg-surface px-4 text-[15px] font-semibold text-ink2"
            >
              재발급
            </button>
          </div>
