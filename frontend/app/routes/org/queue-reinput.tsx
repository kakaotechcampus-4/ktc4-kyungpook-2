...
import { getBlockedQueue, resolveBlockedItem } from "@/lib/api";

export async function clientLoader() {
  return { items: await getBlockedQueue() };
}

export default function ReinputQueuePage() {
  const { items } = useLoaderData<typeof clientLoader>();
  const revalidator = useRevalidator();

  async function resolve(id: string, action: "reupload" | "hold") {
    await resolveBlockedItem(id, action);
  ...
  return (
    <>
          ...
          {items.map((item) => (
            <Card key={item.id}>
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => resolve(item.id, "reupload")}
                  className="tap rounded bg-accent px-4 text-[15px] font-semibold text-white hover:bg-accentink"
                >
                  수정한 원본 다시 올리기
                </button>
                <button
                  onClick={() => resolve(item.id, "hold")}
      ...
      <div className="mt-5">
        <Note>
          원본은 수정하거나 삭제하지 않습니다. 고친 내용을{" "}
          <b className="font-semibold">새 기록으로 추가</b>합니다 (append-only).
        </Note>
      </div>
    </>
  );
