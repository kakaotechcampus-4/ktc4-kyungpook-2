...
import { Card, ConfidenceWarning, EmptyState, Note } from "@/components/ui";
...
export function MatchingQueueClient({ items }: { items: MatchingItem[] }) {
  ...
  return (
    <>
      ...
      <Card>
        ...
        {item.candidates.length > 0 ? (
          <fieldset className="mb-4 flex flex-col gap-2">
            ...
            {item.candidates.map((c) => (
              ...
              >
                <input type="radio" name={`cand-${item.id}`} className="size-4" />
                <span
                  aria-hidden
        ...
        <div className="mb-4">
          <Note>
            낮은 확신도의 기록은 자동으로 확정되지 않습니다. 사람이 직접 아이를 선택해주세요.
          </Note>
        </div>
