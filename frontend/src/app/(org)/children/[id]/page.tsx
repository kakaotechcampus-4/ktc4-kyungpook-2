...
export default async function ChildDetailPage({
  ...
  const [child, timeline] = await Promise.all([getChild(id), getTimeline(id)]);
  if (!child) notFound();
  ...
  return (
    <>
      <PageHeader
        title={`아동 관리 · ${child.name}`}
        description="승인된 요약이 날짜순으로 쌓인 Child Context 입니다"
        right={
          <Link
            href="/chat"
      ...
      {withEntry.length === 0 ? (
        <>
          <EmptyState
            ...
            description="기록을 업로드하고 Gate 1 에서 승인하면 여기에 날짜순으로 쌓입니다. 승인되지 않은 기록은 표시되지 않습니다."
          />
          <div className="mt-4">
            <Note>
              {child.status === "pending_consent"
                ? "보호자 동의가 완료되기 전에는 기록을 올릴 수 없습니다."
                : "기록 업로드 화면에서 첫 기록을 올려보세요."}
            </Note>
          </div>
        </>
      ) : (
