...
export function Gate1Client({ items }: { items: SummaryItem[] }) {
  ...
  const settle = (id: string, status: SummaryItem["gate1Status"], reason?: string) =>
    setQueue((q) =>
      q.map((s) => (s.id === id ? { ...s, gate1Status: status, rejectReason: reason } : s)),
    );
    ...
    <div className="flex flex-col gap-5">
      {pending.length === 0 && rejected.length === 0 ? (
        <EmptyState
          ...
          description="기록을 업로드하면 매칭 · 검증 · 요약을 거쳐 여기로 옵니다."
        />
      ) : null}
      ...
      {pending.map((s) => {
        ...
        return (
          <Card key={s.id}>
            ...
            {rejecting === s.id ? (
              <div className="mb-4 flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => {
                      settle(s.id, "rejected", "선생님이 입력한 반려 사유");
                      setRejecting(null);
                    }}
              ...
              <div className="mb-3 flex flex-wrap gap-2">
                ...
                <button
                  onClick={() =>
                    editing === s.id ? settle(s.id, "approved") : setEditing(s.id)
                  }
            ...
            <Note>
              반려하면 이 요약은 저장되지 않습니다. 업로드한 기록으로 다시 작성됩니다.
            </Note>
          </Card>
        );
      })}
