...
변경이 기존 문서의 사실·규약·실행 절차에 영향을 주면 관련 원본 문서를 같은 변경에서 갱신한다.

새 문서는 기존 문서로 관리할 수 없는 지속적인 팀 규약, 기능 명세, 기술 결정, 운영 절차가
필요할 때만 생성한다. 문서의 위치, 이름, 범위가 명확하면 생성하고 `docs/README.md`에
연결한다. 범위나 소유 문서가 불명확하면 새 문서를 만들기 전에 사용자에게 확인한다.
